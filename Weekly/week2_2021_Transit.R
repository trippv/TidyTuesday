...
ggplot(transit_country, aes(x = sum_real_cost_bll, y = country.name.en, fill = mean_cost_km))+
  
  geom_bar(stat = "identity", width = 0.75) + 
  
  
  geom_point(aes(x = sum_real_cost_bll + 20, y = country.name.en, col = mean_cost_km), 
             size = 8, alpha = 0.4) +
  ...
  geom_text(aes(x = sum_real_cost_bll + 20, label=n_line), col = "black", size = 4)+
  ...
  labs(caption = "Data Source:  Transit Costs Project (https://transitcosts.com)",
       ...
       fill = "Average cost of project per KM \n (in millions of USD)",
